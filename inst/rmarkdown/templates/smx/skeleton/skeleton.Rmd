...
DT::renderDataTable({
  d <- 
    kv.this.year %>% 
    filter(tegund == as.numeric(input$Species),
           (!ok.osl | !ok.sl | !ok.sl.osl | !ok.kirtlar.osl | !ok.lifur.osl)) %>%
    left_join(st %>% select(synis_id, leidangur, stod)) %>% 
    select(leidangur, stod, index, nr, lengd, oslaegt, slaegt, kynfaeri, lifur, ok.osl:ok.lifur.osl)
  d %>% 
    DT::datatable(extensions = 'Scroller',
                  rownames = FALSE,
                  options = list(deferRender = TRUE,
...
DT::renderDataTable({
  d <- 
    kv.this.year %>% 
    filter((!ok.osl | !ok.sl | !ok.sl.osl | !ok.kirtlar.osl | !ok.lifur.osl)) %>%
    left_join(st %>% select(synis_id, leidangur, stod)) %>% 
    select(leidangur, stod, index, tegund, nr, lengd, oslaegt, slaegt, kynfaeri, lifur, magi, ok.osl:ok.lifur.osl)
  d %>% 
    DT::datatable(extensions = 'Scroller',
                  rownames = FALSE,
                  options = list(deferRender = TRUE,
