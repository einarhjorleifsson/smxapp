...
renderPlot({
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::scale_x_log10(breaks = c(seq(5, 50, by = 5), seq(60, 100, by = 10), 120, 140, 160, 200), lim = range(d$lengd, na.rm = TRUE)) +
...
renderPlot({
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::scale_x_log10(breaks = c(5, 10, 15, 30, 60, 100), lim = range(d$lengd, na.rm = TRUE)) +
    ggplot2::scale_y_log10(breaks = c(5, 25, 250, 500, 1000, 5000, 10000), lim = range(d$slaegt, na.rm = TRUE))
})
...
leaflet::renderLeaflet({
  p <-
    leaflet::leaflet(stadlar.rallstodvar.sf %>% 
              dplyr::mutate(col = dplyr::case_when(gid == 77 ~ "cyan",
                                     TRUE ~ "pink"))) %>% 
    leaflet::addTiles() %>% 
    leaflet::addPolylines(color = ~col, weight = 10)
  years <- as.character(now.year:(now.year - 3))
  for(i in 1:length(years)) {
    ...
    if(i == 1) {
      p <-
        ...
        leaflet::addPolylines(data = st.done.sf[st.done.sf$year == years[i],] %>% 
                       dplyr::mutate(col = dplyr::case_when(gid == 77 ~ "blue",
                                                     TRUE ~ "red")),
                     color = ~col,
                     group = years[i],
