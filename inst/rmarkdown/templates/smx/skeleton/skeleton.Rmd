...
### Troll stuff - síðustu 20 stöðvar
...
renderPlot({
  last.20 %>% 
    ...
    geom_point(data = last.20 %>% filter(ar >= 2014, leidangur == input$Leidangur), aes(colour = factor(ar), group = factor(ar))) +
    geom_line(data = last.20 %>% filter(ar >= 2014, leidangur == input$Leidangur), aes(colour = factor(ar), group = factor(ar))) +
